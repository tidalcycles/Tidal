...
controls = intercalate "\n" $ map fs $ sortBy (compare `on` (\(_, x, _) -> x)) genericParams
  where
    ...
    control (t, name, desc) =
      concat
        [ "-- | " ++ desc ++ "\n",
          ...
          "\"\n",
          name,
          "Take :: String -> [Double] -> ControlPattern\n",
          name,
          "Take name xs = pStateListF \"",
          name,
          "\" name xs\n",
          counters t name
        ]
    ...
    counters "f" name =
      concat
        [ name,
          "Count :: String -> ControlPattern\n",
          name,
          "Count name = pStateF \"",
          name,
          "\" name (maybe 0 (+1))\n",
          name,
          "CountTo :: String -> Pattern Double -> Pattern ValueMap\n",
          name,
          "CountTo name ipat = innerJoin $ (\\i -> pStateF \"",
          name,
          "\" name (maybe 0 ((`mod'` i) . (+1)))) <$> ipat\n\n"
        ]
    counters _ _ = ""
    bus (t, name, desc)
      | elem name nobus =
          concat
            [ name,
              ...
              " -> ControlPattern\n",
              name,
              "bus _ _ = error $ \"Control parameter '" ++ name ++ "' can't be sent to a bus.\"\n"
            ]
      | otherwise =
          concat
            [ name,
              ...
              "\" busid)\n",
              name,
              "recv :: Pattern Int -> ControlPattern\n",
              name,
              "recv busid = pI \"^",
              name,
              "\" busid\n"
