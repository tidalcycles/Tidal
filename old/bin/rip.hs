...
play h conn n =
  do
    let tmpfn = printf "tmp-%03d.wav" n
        fn = printf "note-%03d.wav" n
    forkIO $ do
      rawSystem "ecasound" ["-t:" ++ (show time), "-i", "jack,system", "-o", tmpfn]
      return ()
    threadDelay 500000
    ...
    forkIO $ do
      rawSystem "sox" [tmpfn, fn, "silence", "1", "0", "-55d", "reverse", "silence", "1", "0", "-55d", "reverse"]
      rawSystem "rm" [tmpfn]
      return ()
    return ()
