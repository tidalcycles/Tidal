...
module Sound.Tidal.Mininotation where
...
import           Sound.Tidal.Signal.Input
...
tShow (TPat_EnumFromTo a b) = "join $ fromTo <$> (" ++ tShow a ++ ") <*> (" ++ tShow b ++ ")"
...
toPat = \case
   TPat_Atom (Just loc) x -> setMetadata (mempty {metaSrcPos = [loc]}) $ pure x
   ...
   TPat_EnumFromTo a b -> join $ (fromTo <$> toPat a) Sound.Tidal.Pattern.<* toPat b
