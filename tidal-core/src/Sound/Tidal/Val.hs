...
module Sound.Tidal.Val where

import           Data.Word         (Word8)
import           Sound.Tidal.Types

-- | Apply one of three functions to a Value, depending on its type
applyFIRS :: (Double -> Double) -> (Int -> Int) -> (Rational -> Rational) -> (String -> String) -> Val -> Val
...
fNum2 :: (Int -> Int -> Int) -> (Double -> Double -> Double) -> Val -> Val -> Val
