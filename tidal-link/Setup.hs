...
buildTidalLink pdesc lbinfo =
  ...
  where
    ...
    args (Platform _ Windows) =
      ...
      , "-Ilink/modules/asio-standalone/asio/include", "src/c/link_wrapper.cpp"
      , "-DLINK_PLATFORM_WINDOWS=1", "-Dmingw32_BUILD_OS=1"
      , "-Dx86_64_BUILD_ARCH=1", "-Dmingw32_HOST_OS=1", "-Dx86_64_HOST_ARCH=1"
      , "-Wno-multichar", "-g", "-liphlpapi", "-lwinmm"
      , "-lws2_32", "-lstdc++"]
    args _ = []
