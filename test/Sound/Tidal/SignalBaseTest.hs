...
import           Sound.Tidal.Pattern (atom, fastCat)
...
{-
    ...
    describe "unwrap" $ do
      it "preserves inner structure" $ do
        it "one" $
          (queryArc (unwrap $ atom (fastCat [atom "a", atom ("b" :: String)])) (Arc 0 1))
          `shouldBe` (queryArc (fastCat [atom "a", atom "b"]) (Arc 0 1))
        it "two" $
          (queryArc (unwrap $ atom (fastCat [atom "a", atom "b", fastCat [atom "c", atom ("d" :: String)]])) (Arc 0 1))
          `shouldBe` (queryArc (fastCat [atom "a", atom "b", fastCat [atom "c", atom "d"]]) (Arc 0 1))
      it "preserves outer structure" $ do
        it "one" $
          (queryArc (unwrap $ fastCat [atom $ atom "a", atom $ atom ("b" :: String)]) (Arc 0 1))
          `shouldBe` (queryArc (fastCat [atom "a", atom "b"]) (Arc 0 1))
        it "two" $
          (queryArc (unwrap $ fastCat [atom $ atom "a", atom $ atom "b", fastCat [atom $ atom "c", atom $ atom ("d" :: String)]]) (Arc 0 1))
          `shouldBe` (queryArc (fastCat [atom "a", atom "b", fastCat [atom "c", atom "d"]]) (Arc 0 1))
    ...
    describe "rotR" $ do
      it "works over two cycles" $
       property $ comparePD (Arc 0 2) (0.25 ~> atom "a") (0.25 `rotR` atom ("a" :: String))
      it "works over one cycle" $
       property $ compareP (Arc 0 1) (0.25 ~> atom "a") (0.25 `rotR` atom ("a" :: String))
      it "works with zero width queries" $
       property $ compareP (Arc 0 0) (0.25 ~> atom "a") (0.25 `rotR` atom ("a" :: String))
    ...
    describe "withResultArc" $ do 
     it "apply given function to the Arcs" $ do
      let p = withResultArc (+5) (stripMetadata $ fast "1 2" "3 4" :: Pattern Int) 
    ...
    describe "filterValues" $ do 
     it "remove Events above given threshold" $ do 
       let fil = filterValues (<2) $ fastCat [atom 1, atom 2, atom 3] :: Pattern Time 
    ...
    describe "filterWhen" $ do 
      ...
      it "filter above given threshold" $ do 
        let fil = stripMetadata $ filterWhen (>0.5) $ struct "t*4" $ (tri :: Pattern Double) + 1
        let res = queryArc fil (Arc 0.5 1.5)
