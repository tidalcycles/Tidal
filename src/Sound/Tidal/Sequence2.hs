...
-- TODO - optimise
seqSplitAt :: Time -> Sequence a -> Maybe (Sequence a)
seqSplitAt t s = do a <- seqTake t s
                    b <- seqDrop t s
                    return (a,b)

seqSplitAt' :: Time -> Sequence a -> Sequence a
seqSplitAt' t s = fromMaybe (Gap 0) $ seqSplitAt t s -- TODO - error handling..
