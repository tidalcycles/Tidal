...
toScreen (x, y) =
  ( floor (x * (fromIntegral screenWidth)),
    floor (y * (fromIntegral screenHeight))
  )

toScreen16 :: (Float, Float) -> (Int16, Int16)
toScreen16 (x, y) =
  ( fromIntegral $ floor (x * (fromIntegral screenWidth)),
...
applySurface x y src dst clip = blitSurface src clip dst offset
  where
    offset = Just Rect {rectX = x, rectY = y, rectW = 0, rectH = 0}
  ...
  | otherwise =
      do
        let pix = (colourToPixel c)
            steps = [t, (t + step) .. (t + o)]
            coords =
              map (\s -> (floor $ x + (r * cos (s)), floor $ y + (r * sin (s)))) steps
...
drawPat (x, y) (r, r') p screen beat = mapM_ drawEvents es
  where
    es = map (\(_, (s, e), evs) -> ((max s pos, min e (pos + 1)), evs)) $ arc (segment p) (pos, pos + 1)
...
splitEs _ [] = []
splitEs t ((ev@(a, (s, e), (h, v))) : es)
  | t > s && t < e = (a, (s, t), (h, v)) : (a, (t, e), (False, v)) : (splitEs t es)
...
textSize text font =
  do
    message <- renderTextSolid font text (Color 0 0 0)
